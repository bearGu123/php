...
function zwmp($ymd,$hour,$sex,$judge){
    ...
    if($jieqi==$jieqi2){
        $jieqiArrJudge=array('立春','惊蛰','清明','立夏','芒种','小暑','立秋','白露','寒露','立冬','大雪','小寒');
        foreach($jieqiArrJudge as $key=>$value){
            if($jieqi==$value){
                $m-=1;
                if($m==0)$m=12;
                break;
...
function save(){
        $data=[
            'uid'=>cookie('uid'),	   //用户id
            'phone'=>cookie('phone'),	      //手机号码
            'username'=>cookie('znickname'),	   //姓名
            'datetype'=>cookie('datatype'), //阳历0   阴历1
            'sex'=>cookie('zsex'), //0 女 1 男
            'addr'=>cookie('placebirth'),
            'bir1'=>cookie('zymd'),  //日期 2017-10-01
            'bir2'=>cookie('zhour'),  //生辰
        ];
//        $json=json_encode($data,JSON_UNESCAPED_UNICODE);
        $url = "http://sm.ddznzj.com/sm/userhp/insert";
        $ch = curl_init();
